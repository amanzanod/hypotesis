...
<script>
    ...
    export default {
        ...
        beforeMount () {
            ...
            if (!this.is_new) {
                this.axios
                    .get(HYP_CONTEXT_MASTER + this.alias + '/?format=json')
                    .then(response => {
        ...
        methods: {
            onSubmit(evt) {
                ...
                if (this.is_new) {
                    ...
                    fetch(HYP_CONTEXT_MASTER, requestOptions)
                        .then(response => {
                            console.log(response);
                            if (response.status < 300) {
                                this.text_modal = 'Se ha creado el máster';
                                this.$router.replace({ name: 'Masters'});
                            } else {
                                this.text_modal = 'No se ha podido crear el máster';
                                this.showModal();
                            }
                        })
                        ...
                        .catch(error => {
                            console.log(error);
                            this.text_modal = 'Hubo un error en la creación del máster';
                            this.showModal();
                        });
                    ...
                    fetch(HYP_CONTEXT_MASTER + this.alias + '/', requestOptions)
                        .then(response => {
                            console.log(response);
                            if (response.status < 300) {
                                this.text_modal = 'Se ha actualizado el máster';
                                this.$router.replace({ name: 'Masters'});
                            } else {
                                this.text_modal = 'No se ha podido actualizar el máster';
                                this.showModal();
                            }
                        })
                        .then(result => console.log(result))

...
<script>
    ...
    export default {
        ...
        beforeMount () {
            ...
            if (!this.is_new) {
                this.axios
                    .get(HYP_CONTEXT_GRADE + this.alias + '/?format=json')
                    .then(response => {
        ...
        methods: {
            onSubmit(evt) {
                ...
                if (this.is_new) {
                    ...
                    fetch(HYP_CONTEXT_GRADE, requestOptions)
                        .then(response => {
                            console.log(response);
                            if (response.status < 300) {
                                this.text_modal = 'Se ha creado el grado';
                                this.$router.replace({ name: 'Grades'});
                            } else {
                                this.text_modal = 'No se ha podido crear el grado';
                                this.showModal();
                            }
                        })
                        ...
                        .catch(error => {
                            console.log(error);
                            this.text_modal = 'Hubo un error en la creación del grado';
                            this.showModal();
                        });
                    ...
                    fetch(HYP_CONTEXT_GRADE + this.alias + '/', requestOptions)
                        .then(response => {
                            console.log(response);
                            if (response.status < 300) {
                                this.text_modal = 'Se ha actualizado el grado';
                                this.$router.replace({ name: 'Grades'});
                            } else {
                                this.text_modal = 'No se ha podido actualizar el grado';
                                this.showModal();
                            }
                        })
                        .then(result => console.log(result))
